...
class GB : public I_linkcable_target {
public:
	GB();
	~GB();
   ...
   int load(const void *romdata, unsigned size, unsigned flags = 0);
	...
	unsigned char receive_from_linkcable(unsigned char data);
	bool is_ready();

	I_linkcable_target* get_linked_target() { return this->linked_target; };
	void set_linked_target(I_linkcable_target* target) { this->linked_target = target; };
...
private:
	...
	I_linkcable_target* linked_target;
};
}
