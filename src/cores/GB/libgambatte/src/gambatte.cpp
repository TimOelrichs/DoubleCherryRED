...
unsigned char GB::receive_from_linkcable(unsigned char received_data) {

	unsigned char out_data = p_->cpu.mem_.getSB();
	//p_->cpu.mem_.startSerialTransfer(p_->cpu.getCycleCounter(), received_data, p_->cpu.isCgb());
	//p_->cpu.mem_.setSB(received_data);
	//p_->cpu.mem_.setSC(p_->cpu.mem_.getSC() & 0x7F);
	//p_->cpu.mem_.intreq_.flagIrq(8);
	return out_data;

}
// link ready SC == 0x80
bool GB::is_ready() { return (p_->cpu.mem_.getSC() & 0x80) == 0x80 ; }
...
/*
unsigned char GB::send_over_linkcable(unsigned char send_data) {

	return this->get_linked_target() ? this->get_linked_target()->receive_from_linkcable(send_data) : 0xFF;
}
*/