...
#define SB  ioamhram_[0x101]
#define SC  ioamhram_[0x102]
...
void Memory::checkSerial(unsigned long const cc) {
	...
	if ((serial_io_ != 0) &&
		...
		unsigned char data;
		bool fastCgb;
		if (serial_io_->check(SB, data, fastCgb)) {
...
void Memory::updateSerial(unsigned long const cc) {
	if (intreq_.eventTime(intevent_serial) != disabled_time) {
		if (intreq_.eventTime(intevent_serial) <= cc) {
			...
			bool fire = ((SC & 0x80) == 0x80);
			SB = ((SB << serialCnt_) |(serialize_value_ >> (8 - serialCnt_))) & 0xFF;
			
#else
         SB = (((SB + 1) << serialCnt_) - 1) & 0xFF;
#endif
...
#ifdef HAVE_NETWORK
			                                    serialize_is_fastcgb_);
			SB = ((SB << (serialCnt_ - targetCnt)) |
					    (serialize_value_ >> (8 - (serialCnt_ - targetCnt)))) & 0xFF;
#else
                                             SC & isCgb() * 2);
         SB = (((SB + 1) << (serialCnt_ - targetCnt)) - 1) & 0xFF;
#endif
...
void Memory::nontrivial_ff_write(unsigned const p, unsigned data, unsigned long const cc) {
	...
	case 0x00:	// P1 (control pad)
		...
		return;
	case 0x01:	 // SB (sending and receiving serial communication data)	
		updateSerial(cc);
		break;
	case 0x02: // SC (serial control)
		...
		if ((data & 0x81) == 0x81)
		{
			...
			if (serial_io_ != 0)
				receivedByte = serial_io_->send(SB, (data & isCgb() * 2));
